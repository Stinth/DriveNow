...
def series_to_supervised(d, n_in=3, n_out=1, dropnan=True):
    """
...
def splitData(d):
    X_filter = series_to_supervised(d, 1, 1)
    target = X_filter['var1(t)']

    Index_pred = X_filter.index
    predictors = X_filter.drop('var1(t)', axis=1).reset_index()
    predictors = predictors.drop(['FromZoneID', 'Start_tidspunkt'], axis=1)

    # autocorrelation_plot(target)

    return target, predictors, Index_pred
...
def Autoregression(predictors):
    ...
    model = AR(data)
    model_fit = model.fit()
    yhat_AR = model_fit.predict(len(data), len(data))

    return yhat_AR


def Arima(predictors):
    data = predictors
    # fit model
    model = SARIMAX(data, order=(1, 1, 1), seasonal_order=(1, 1, 1, 1))
    model_fit = model.fit(disp=False)
    # make prediction
    yhat_Arima = model_fit.predict(len(data), len(data))

    return yhat_Arima


def Holtsmooth(predictors):
    # contrived dataset
    data = predictors
    # fit model
    ...
    model_fit = model.fit()
    # make prediction
    yhat_Holts = model_fit.predict(len(data), len(data))
